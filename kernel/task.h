...
struct mm_struct
{
    ...
    unsigned long stack_brk,end_brk;
    unsigned long stack_stack;
};
...
union task_union init_task_union __attribute__((__section__ (".data.init_task"))) = {INIT_TASK(init_task_union.stack)};
...
do{                                 \
    __asm__ __volatile__ ( "pushq %%rbp     \n\t"   \
                           ...
                           :"m"(next->thread->rsp),"m"(next->thread->rsp),"D"(prev),"S"(next)   \
                           :"memory"    \
                            );      \
