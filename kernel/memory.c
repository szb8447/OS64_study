...
unsigned long page_init(struct Page* page,unsigned long flags)
{
    if(!page->attribute)
    {
        *(memory_management_struct.bits_map + ((page->PHY_address >> PAGE_2M_SHIFT)
        >> 6)) |= 1UL << (page->PHY_address >> PAGE_2M_SHIFT) % 64;
...
void init_memory()
{
    ...
    memory_management_struct.pages_length = ((TotalMem >> PAGE_2M_SHIFT) * sizeof(struct Page) + 
    ...
    color_printk(ORANGE,BLACK,"bits_map:%#0181x,bits_size:%#0181x,bits_length:%#0181x\n",
    memory_management_struct.bits_map,memory_management_struct.bits_size,memory_management_struct.bits_length);

    color_printk(ORANGE,BLACK,"pages_struct:%#0181x,pages_size:%#0181x,pages_length:%#0181x\n",
    memory_management_struct.pages_struct,memory_management_struct.pages_size,memory_management_struct.pages_length);

    color_printk(ORANGE,BLACK,"zones_struct:%#0181x,zones_size:%#0181x,zone_length:%#0181x\n",
    ...
    for (i = 0; i < memory_management_struct.zones_size; i++)//need rewrite in the future
    {
        ...
        color_printk(ORANGE,BLACK,"zone_start_address:%#0181x,zone_end_address:%#0181x,zone_length:%#0181x,pages_group:%#0181x,pages_length:%#0181x\n",z->zone_start_address,z->zone_end_address,z->zone_length,z->pages_group,z->pages_length);

        if(z->zone_start_address == 0x1000000000)
        {
    ...
    color_printk(ORANGE,BLACK,"start_code:%#0181x,end_code:%#0181x,end_data:%#0181x,end_brk:%#0181x,end_of_struct:%#0181x\n",memory_management_struct.start_code,memory_management_struct.end_code,memory_management_struct.end_data,memory_management_struct.end_brk,memory_management_struct.end_of_struct);
    ...
    color_printk(INDIGD,BLACK,"Global_CR3\t:#0181x\n",Global_CR3);
    color_printk(INDIGD,BLACK,"*Global_CR3\t:%#0181x\n",*Phy_To_Virt(Global_CR3) & (~0xff));
    color_printk(PURPLE,BLACK,"**Global_CR3\t:%#0181x",*Phy_To_Virt(*Phy_To_Virt(Global_CR3) & (~0xff)) & (~0xff));
    ...
    for (i = zone_start; i <= zone_end; i++)
    {
        ...
        for (j = start; j <= end; j+= j%64 ? tmp : 64)
        {
            ...
            for (k = shift; k < 64 - shift; k++)
            {
                ...
                0xffffffffffffffffUL : ((1UL << number) - 1))))
                {
                    unsigned long l;
                    page = j + k -1;
                    for (l = 0; l < number; l++)
